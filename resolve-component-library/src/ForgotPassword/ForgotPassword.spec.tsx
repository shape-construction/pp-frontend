...
describe('reset password panel', () => {
  ...
  it('renders the log in link', () => {
    render(<ForgotPassword logInLink="awesome log in page" />);

    expect(screen.getByText('Back to awesome log in page')).toBeInTheDocument();
  });

  it('renders the email field with given value', () => {
    ...
    expect(emailInput).toHaveAttribute('value', 'email@example.com');
  });

  it('renders the email field with given error', () => {
    render(
      <ForgotPassword
        {...defaultProps}
        fieldsErrors={{
  ...
  it('renders the submit feedback message', () => {
    render(
      <ForgotPassword {...defaultProps} submitFeedback="the form was submitted successfully!" />
    );
